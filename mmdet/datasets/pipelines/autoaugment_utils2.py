...
def _apply_multi_bbox_augmentation(image, bboxes, new_masks, prob, aug_func,
...
def _apply_multi_bbox_augmentation_wrapper(image, bboxes, masks, prob, aug_func,
                                           func_changes_bbox, *args):
    ...
    if num_bboxes != 0:
        new_image, new_bboxes = _apply_multi_bbox_augmentation(
            new_image, new_bboxes, new_masks, prob, aug_func, func_changes_bbox, *args)
    return new_image, new_bboxes


def rotate_only_bboxes(image, bboxes, masks, prob, degrees, replace):
    """Apply rotate to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, bboxes, masks, prob, rotate, func_changes_bbox, degrees, replace)
...
def translate_bbox(image, bboxes, masks, pixels, replace, shift_horizontal):
    ...
    Args:
        ...
        bboxes: 2D Tensor that is a list of the bboxes in the image. Each bbox
            ...
            between [0, 1].
        masks: BitmapMasks in mmdetection
    ...
    if shift_horizontal:
        ...
        direction = 'vertical'
    new_maks = masks.translate((image.shape[0], image.shape[1]), pixels, direction)
    return image.astype(np.uint8), new_bboxes, new_maks
...
def shear_with_bboxes(image, bboxes, masks, level, replace, shear_horizontal):
    ...
    if shear_horizontal:
        ...
        direction = 'vertical'
    new_maks = masks.shear((image.shape[0], image.shape[1]), level, direction)
    return image.astype(np.uint8), new_bboxes, new_maks
...
NAME_TO_FUNC = {
        ...
        # pylint:disable=g-long-lambda
        'TranslateX_BBox': lambda image, bboxes, masks, pixels, replace: translate_bbox(
                image, bboxes, masks, pixels, replace, shift_horizontal=True),
        'TranslateY_BBox': lambda image, bboxes, masks, pixels, replace: translate_bbox(
                image, bboxes, masks, pixels, replace, shift_horizontal=False),
        'ShearX_BBox': lambda image, bboxes, masks, level, replace: shear_with_bboxes(
                image, bboxes, masks, level, replace, shear_horizontal=True),
        'ShearY_BBox': lambda image, bboxes, masks, level, replace: shear_with_bboxes(
                image, bboxes, masks, level, replace, shear_horizontal=False),
        # pylint:enable=g-long-lambda
