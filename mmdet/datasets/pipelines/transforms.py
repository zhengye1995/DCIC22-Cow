                ...
                if random.uniform(0, 1) < self.prob:
                    ...
                    for _ in range(n_holes):
                        ...
                        if not self.with_ratio:
                            ...
                            cutout_w = int(self.candidates[index][0] * (bbox[2]-bbox[0]))
                            cutout_h = int(self.candidates[index][1] * (bbox[3]-bbox[1]))

                        x2 = np.clip(x1 + cutout_w, 0, bbox[2]-bbox[0])
                        y2 = np.clip(y1 + cutout_h, 0, bbox[3]-bbox[1])
                        results['img'][y1:y2, x1:x2, :] = self.fill_in
...
class CopyPaste(object):
    ...
    def __call__(self, results):
        if random.uniform(0, 1) < self.prob:
            ...
            results['ann_info']['bboxes'] = np.vstack((list(gt_bboxes), paste_bboxes))
            # print('*'*20)
...
class GridMask(object):
    def __init__(self, use_h, use_w, rotate=1, offset=False, ratio=0.5, mode=0, prob=0.5):
        self.use_h = use_h
        self.use_w = use_w
        self.rotate = rotate
        self.offset = offset
...
@PIPELINES.register_module()
class Rotate90:
    """Apply Rotate90 Transformation to image (and its corresponding bbox, mask,
    segmentation).
    ...
    def _rotate_img(self, results):
        ...
        Args:
            ...
            scale (int | float): Isotropic scale factor. Same in
                ``mmcv.imrotate``.
        """
        is_neg = np.random.randint(low=0, high=2)
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            results['before_rotate_img_shape'] = img.shape
            if is_neg == 0:
                img_rotated = np.ascontiguousarray(np.rot90(img, -1))
                is_neg = False
            else:
                img_rotated = np.ascontiguousarray(np.rot90(img, 1))
                is_neg = True
    ...
    def _rotate_bboxes(self, results, is_neg=False):
        """Rotate the bboxes."""
        h, w, c = results['before_rotate_img_shape']
        for key in results.get('bbox_fields', []):
            min_x, min_y, max_x, max_y = np.split(
                results[key], results[key].shape[-1], axis=-1)
            if is_neg:
                new_min_x, new_min_y, new_max_x, new_max_y = min_y, w - max_x, max_y, w - min_x
            else:
                new_min_x, new_min_y, new_max_x, new_max_y = h - max_y, min_x, h - min_y, max_x
            results[key] = np.stack([new_min_x, new_min_y, new_max_x, new_max_y],
                                    axis=-1).astype(results[key].dtype)
            results[key] = np.squeeze(results[key], 1)
    ...
    def _rotate_masks(self,
        ...
        raw_h, raw_w, _c = results['before_rotate_img_shape']
        ...
        for poly_per_obj in gt_masks.masks:
            ...
            for p in poly_per_obj:
                ...
                # print(p)
                if is_neg:
                    p[0::2], p[1::2] = tp[1::2], raw_w - tp[0::2]

                else:
                    p[0::2], p[1::2] = raw_h - tp[1::2], tp[0::2]

                rotated_poly.append(p)
            rotated_masks.append(rotated_poly)
        rotated_masks = PolygonMasks(rotated_masks, h, w)
    ...
    def _rotate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            ...
            if is_neg == 0:
                results[key] = np.ascontiguousarray(np.rot90(seg, -1))
            else:
                results[key] = np.ascontiguousarray(np.rot90(seg, 1))
    ...
    def __call__(self, results):
        ...
        if random.uniform() > self.prob:
            return results
        is_neg = self._rotate_img(results)
        self._rotate_bboxes(results, is_neg)
        self._rotate_masks(results, is_neg)
        self._rotate_seg(results, is_neg)
        return results
...
@PIPELINES.register_module()
class AutoAugmentOffical(object):
    def __init__(self, autoaug_type="v1"):
        """
        Args:
            autoaug_type (str): autoaug type, support v0, v1, v2, v3, test
        """
        super(AutoAugmentOffical, self).__init__()
