...
class FCNMaskHead(BaseModule):
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
                      ori_shape, scale_factor, rescale, return_prob=False):
        ...
        for inds in chunks:
            ...
            if return_prob:
                im_prob[(inds,) + spatial_inds] = masks_chunk
        ...
        for i in range(N):
            ...
            if return_prob:
                cls_probs[labels[i]].append(im_prob[i].detach().cpu().numpy().astype(np.float16))
        if return_prob:
            return cls_segms, cls_probs
        return cls_segms
