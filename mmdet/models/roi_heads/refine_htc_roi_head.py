...
from .mask_heads.htc_mask_head import HTCMaskHead
from .mask_heads.refine_mask_head import SimpleRefineMaskHead
from mmdet.models.losses.cross_entropy_loss import generate_block_target


@HEADS.register_module()
class RefineHybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(RefineHybridTaskCascadeRoIHead,
    ...
    def _mask_forward_train(self,
        ...
        if self.mask_info_flow:
            ...
            for i in range(stage):
                if isinstance(self.mask_head[i], SimpleRefineMaskHead):
                    # TODO new head refine_htc_mask_head
                    last_feat = self.mask_head[i](
                        mask_feats, last_feat, x[0], pos_rois,
                        torch.cat(pos_labels), return_logits=False)
                else:
                    last_feat = self.mask_head[i](
                        mask_feats, last_feat, return_logits=False)
            if isinstance(mask_head, SimpleRefineMaskHead):
                mask_pred = mask_head(mask_feats, last_feat, x[0], pos_rois,
                                      torch.cat(pos_labels), return_feat=False)
            else:
                mask_pred = mask_head(mask_feats, last_feat, return_feat=False)
        else:
            if isinstance(mask_head, SimpleRefineMaskHead):
                mask_pred = mask_head(mask_feats, x[0], pos_rois,
                                      torch.cat(pos_labels), return_feat=False)
            else:
        ...
        if isinstance(mask_head, SimpleRefineMaskHead):
            ...
            stage_mask_targets = mask_head.get_targets(pos_bboxes, pos_assigned_gt_inds, gt_masks)
            loss_mask = mask_head.loss(mask_pred, stage_mask_targets)
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                for i in range(self.num_stages):
                    ...
                    if self.mask_info_flow:
                        if isinstance(mask_head, SimpleRefineMaskHead):
                            stage_instance_preds, last_feat = mask_head(mask_feats, last_feat,
                                                             x[0], rois, torch.cat(det_labels))[1:]
                            for idx in range(len(stage_instance_preds) - 1):
                                instance_pred = stage_instance_preds[idx].squeeze(1).sigmoid() >= 0.5
                                non_boundary_mask = (
                                        generate_block_target(instance_pred, boundary_width=1) != 1).unsqueeze(1)
                                non_boundary_mask = F.interpolate(
                                    non_boundary_mask.float(),
                                    stage_instance_preds[idx + 1].shape[-2:], mode='bilinear',
                            ...
                            mask_pred = stage_instance_preds[-1]
                        else:
                            mask_pred, last_feat = mask_head(mask_feats, last_feat)
                    else:
                        if isinstance(mask_head, SimpleRefineMaskHead):
                            stage_instance_preds = mask_head(mask_feats, x[0], rois, torch.cat(det_labels))[1:]
                            for idx in range(len(stage_instance_preds) - 1):
                                instance_pred = stage_instance_preds[idx].squeeze(1).sigmoid() >= 0.5
                                non_boundary_mask = (
                                            generate_block_target(instance_pred, boundary_width=1) != 1).unsqueeze(1)
                ...
                for x, img_meta, semantic in zip(img_feats, img_metas,
                    ...
                    for i in range(self.num_stages):
                        ...
                        if self.mask_info_flow:
                            if isinstance(mask_head, SimpleRefineMaskHead):
                                stage_instance_preds, last_feat = mask_head(mask_feats, last_feat,
                                                                            x[0], mask_rois, det_labels)
                                stage_instance_preds = stage_instance_preds[1:]
                                for idx in range(len(stage_instance_preds) - 1):
                                    instance_pred = stage_instance_preds[idx].squeeze(1).sigmoid() >= 0.5
                                    non_boundary_mask = (
                                            generate_block_target(instance_pred, boundary_width=1) != 1).unsqueeze(1)
                                ...
                                mask_pred = stage_instance_preds[0] # -1
                                # print('%' * 50)
                                # for mask_ in stage_instance_preds:
                                #     print(mask_.size())
                                # print('%' * 50)
                            else:
                                mask_pred, last_feat = mask_head(mask_feats, last_feat)
                            ...
                            if isinstance(mask_head, SimpleRefineMaskHead):
                                stage_instance_preds = mask_head(mask_feats, x[0], mask_rois, det_labels)[1:]
                                stage_instance_preds = stage_instance_preds[1:]
                                for idx in range(len(stage_instance_preds) - 1):
                                    instance_pred = stage_instance_preds[idx].squeeze(1).sigmoid() >= 0.5
                                    non_boundary_mask = (
