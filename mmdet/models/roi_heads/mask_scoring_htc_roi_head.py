...
class MaskScoringHybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(MaskScoringHybridTaskCascadeRoIHead,
    ...
    def _mask_forward_train(self,
        ...
        mask_iou_pred = self.mask_iou_head(mask_feats,
                                           pos_mask_pred)
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                for i in range(self.num_stages):
                    ...
                    mask_iou_preds = mask_iou_pred.split(num_bbox_per_img, 0)
                    aug_mask_scores.append(mask_iou_preds)
                ...
                for i in range(num_imgs):
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        merged_mask = merge_aug_masks(
                            ...
                            rcnn_test_cfg)
                        merged_mask_score = np.mean(aug_mask_scores, axis=0)
                        ...
                        mask_score = self.mask_iou_head.get_mask_scores(
                            merged_mask_score[i], det_bboxes[i], det_labels[i])
                        segm_results.append(segm_result)
                        mask_scores.append(mask_score)
            ms_segm_result['ensemble'] = segm_results
            ms_segm_result['ensemble_score'] = mask_scores

        if self.with_mask:
            results = list(
                zip(ms_bbox_result['ensemble'], list(zip((ms_segm_result['ensemble'], ms_segm_result['ensemble_score'])))))
        else:
            ...
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta, semantic in zip(img_feats, img_metas,
                    ...
                    for i in range(self.num_stages):
                        ...
                        if i == self.num_stages - 1:
                            concat_det_labels = det_labels
                            mask_iou_pred = self.mask_iou_head(
                                last_feat, mask_pred[range(concat_det_labels.size(0)),
                                                      concat_det_labels])

                            # mask_iou_preds = mask_iou_pred.split(num_bbox_per_img, 0)
                            aug_mask_scores.append(mask_iou_pred)

                merged_masks = merge_aug_masks(aug_masks, aug_img_metas,
                                               self.test_cfg)
                merged_mask_score = sum(aug_mask_scores) / float(len(aug_mask_scores))
                ...
                # get mask scores with mask iou head
                mask_score = self.mask_iou_head.get_mask_scores(
                    merged_mask_score, det_bboxes, det_labels)

            return [(bbox_result, (segm_result, mask_score))]
        else:
            return [bbox_result]
