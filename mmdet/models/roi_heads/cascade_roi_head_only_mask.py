...
class CascadeRoIHeadOnlyMask(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(CascadeRoIHeadOnlyMask, self).__init__(
        ...
        with open(test_json, 'r') as f:
            raw_info = json.load(f)['images']
        for info in raw_info:
            self.imageid2name[info['id']] = info['file_name']
            self.imagename2bbox[info['file_name']] = []
        with open(bbox_results_json, 'r') as f:
            annos = json.load(f)
        for anno in annos:
            score = anno['score']
            if score < self.test_cfg.score_thr:
                continue
            im_name = self.imageid2name[anno['image_id']]
            bbox = anno['bbox']
            xmin, ymin, w, h = bbox
            xmax = xmin + w
            ymax = ymin + h
            new_bbox = [xmin, ymin, xmax, ymax, score]
            self.imagename2bbox[im_name].append(new_bbox)
    ...
    def aug_test(self, features, proposal_list, img_metas, rescale=False):
        ...
        det_bboxes = torch.tensor(self.imagename2bbox[img_metas[0][0]['ori_filename']]).cuda()  # test bs=1
        det_labels = torch.tensor([0] * det_bboxes.shape[0])
        bbox_result = bbox2result(det_bboxes, det_labels,
                                  self.bbox_head[-1].num_classes)
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                if self.return_prob:
                    segm_result, seg_prob = segm_result
            if self.return_prob:
                return [(bbox_result, segm_result, seg_prob)]
