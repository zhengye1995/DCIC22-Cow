...
class _CBConvNeXt(BaseModule):
    def __init__(self, net, cb_inplanes, cb_zero_init=True, cb_del_stages=0, **kwargs):
        super(_CBConvNeXt, self).__init__()
        self.cb_zero_init = cb_zero_init
        self.cb_del_stages = cb_del_stages
    ...
    def _get_cb_feats(self, feats, spatial_info):
        ...
        for i in range(self.num_layers):
            if i >= self.cb_del_stages:
                ...
                jrange = 4 - i
                for j in range(jrange):
                    # print(self.cb_linears[i][j])
                    # print(feats[j + i].size())
                    # print(feats[j + i + 1].size())
                    tmp = self.cb_linears[i][j](feats[j + i])
                    tmp = F.interpolate(tmp, size=(h, w), mode='nearest')
                    feeds.append(tmp)
                feed = torch.sum(torch.stack(feeds, dim=-1), dim=-1)
...
class _ConvNeXt(ConvNeXt):
    def __init__(self, **kwargs):
        ConvNeXt.__init__(self, **kwargs)
        self.frozen_stages = 0

    def _freeze_stages(self):
        for i in range(1, self.frozen_stages + 1):
            if hasattr(self, f'downsample_layers{i-1}'):
                m = getattr(self, f'downsample_layers{i-1}')
                m.eval()
                for param in m.parameters():
                    param.requires_grad = False
            if hasattr(self, f'stages{i-1}'):
                m = getattr(self, f'stages{i-1}')
                m.eval()
                for param in m.parameters():
                    param.requires_grad = False
    ...
    def del_layers(self, del_stages):
        self.del_stages = del_stages
        # print(self.downsample_layers[0])
        for i in range(1, self.del_stages + 1):
            # delattr(self, f'downsample_layers{i-1}')
            # delattr(self, f'stages{i-1}')
    ...
    def forward(self, x, cb_feats=None, pre_outs=None):
        ...
        for i in range(4):
            if self.downsample_layers[i] is None:
                x_out = pre_outs[i+1]
                outs.append(x_out)
            else:
                spatial_info.append(x.shape[2:])
                if cb_feats is not None:
                    try:
                        print(torch.tensor(x).size(), torch.tensor(cb_feats[i]).size())
                    except:
                        pass
                    x = x + cb_feats[i]
...
@BACKBONES.register_module()
class CBConvNeXt(_CBConvNeXt):
    def __init__(self, **kwargs):
        super().__init__(net=_ConvNeXt, **kwargs)
